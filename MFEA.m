function data_MFDE = MFEA(Tasks,ini_pop,gen,selection_process,cp,p_il,reps,index,rem,EA)
...
%         end
        ...
        while generation < gen
            ...
            if EA ==1 %%Differetial Evolution 
                lb=-ones(1,D_multitask);
                ub=ones(1,D_multitask);
                pCR=0.9;
                F=0.5;
                ...
                for nt = 1:no_of_tasks
                    ...
                    for i = 1 : length(P)/2     
                        ...
                        if (population(p1).skill_factor == population(p2).skill_factor)
                            ...
                            if rand(1) < 0.1 
                                child(count)=mutate(child(count),child(count),D_multitask,sigma);
                                child(count+1)=mutate(child(count+1),child(count+1),D_multitask,sigma);
                            end      
            ...
            if(cp<0 && generation>=1)
                ...
                for nt = 1:no_of_tasks
                    ...
                    for j =1:length(V)
                        ...
                        [test,b] = evaluate(test,Tasks,p_il,no_of_tasks,options);
